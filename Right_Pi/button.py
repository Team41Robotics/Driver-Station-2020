...
class Button:
	...
	def drawButton(self, ctx):
		...
		ctx.create_rectangle(self.x1, self.y1, self.x2, self.y2, tag=self.name, fill=self.color_active)
		# Create the text on the button
		ctx.delete(self.name+"text")
		ctx.create_text(self.x1+(self.width/2), self.y1+(self.height/2), 
			text=self.text_active, tag=self.name+"text", font = ("Comic Sans MS", 14))
