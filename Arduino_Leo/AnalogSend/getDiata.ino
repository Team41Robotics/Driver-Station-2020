...
void getData() {
    ...
    for (int i = 0; i <= NUM_SLIDERS; i++) {
        ...
        int readVar = analogRead(i);
        // To read the bytes, we need to take the first part in one byte, then add to the checksum
        dataBuffer[i*2] = (byte)(readVar & 0xff);
        //checksum[0] ^= sliders[i*2];
        // Then take the rest of it and put it in the next byte
        dataBuffer[(i*2) + 1] = (byte)(readVar & 0x0300 >> 8); // Take the 10 bit value and remove the first 8 bits by shifting them left (out of existence)
    ...
    for (int i = 0; i < NUM_DIALS; i++) {
        ...
        dataBuffer[2*(NUM_SLIDERS+i)] = (byte)(dialRead & 0xff);         // First byte
        dataBuffer[2*(NUM_SLIDERS+i)+1] = (byte)(dialRead & 0x0300 >> 8);   // Last byte
    }
